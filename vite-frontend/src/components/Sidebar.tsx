...
/* ── Mobile bottom tab bar (visible < lg, hidden on lg+) ── */
function MobileBottomNav() {
  const { tab, setTab, locale } = useApp();
  const t = COPY[locale];
  ...
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-50 flex items-center justify-around border-t bg-[var(--card)] px-1 py-1.5 shadow-neo lg:hidden"
    ...
    >
      {TABS.map(({ id, Icon }) => (
        <button
          ...
          onClick={() => setTab(id)}
          title={tabLabel[id]}
          className={`flex flex-col items-center gap-0.5 rounded-xl px-2 py-1 text-[10px] font-semibold transition ${
            tab === id
              ? "text-white bg-neo-accent shadow-neo-sm"
              : "text-neo-muted hover:text-neo-accent"
          }`}
        >
          <Icon className="h-5 w-5" />
          <span className="truncate max-w-[3rem]">{tabLabel[id]}</span>
        </button>
      ))}
    </nav>
  );
...
/* ── Desktop sidebar (hidden < lg, visible on lg+) ── */
export function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Desktop sidebar */}
      <aside
        className={`neo hidden lg:flex w-full shrink-0 flex-col gap-3 p-3 transition-[width] duration-200 lg:sticky lg:top-4 lg:h-[calc(100vh-2rem)] lg:overflow-y-auto ${
      ...
      >
          ...
          {TABS.map(({ id, Icon }) => (
            <button
              ...
              className={`flex items-center gap-2.5 rounded-2xl px-2.5 py-2 text-sm font-semibold transition ${
                sidebarOpen ? "w-full text-left" : "justify-center w-full"
        ...
        <div className={`mt-auto flex gap-1 ${sidebarOpen ? "flex-col" : "flex-col"}`}>
